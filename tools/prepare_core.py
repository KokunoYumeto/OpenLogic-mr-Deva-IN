...
base=base.replace('संच आणि संबंध','संच, संबंध, फलने आणि संचांचे आकारमान').replace('दोन संपूर्ण प्रकरणे','तीन संपूर्ण प्रकरणे आणि चौथ्या प्रकरणाचे सहा विभाग').replace('OLP-0019, एकूण 16 विभाग. उर्वरित 706','OLP-0033, एकूण 30 स्रोत-एकके आणि 26 वाचक-विभाग. उर्वरित 692')
...
absent={'sth:choice::chap','sfr:siz:enm-alt:sec'};decisions=[]
def conditionals(t):
...
\item संचांच्या आकारमानाच्या अपूर्ण प्रकरणातील OLSIZ-001 ते OLSIZ-005
या पाच गोठवलेल्या-स्रोत निष्कर्षांचे स्पष्टीकरण संबंधित परिच्छेदांलगत
स्वतंत्र ``स्रोतदुरुस्ती'' नोंदींमध्ये दिले आहे. मूळ इंग्रजी बाइट्स
बदललेले नाहीत.
'''
notes=notes.replace(r'\end{enumerate}',newnotes+r'\end{enumerate}',1)
out=before+'\n'.join(chunks)+'\n'+notes
assert 'एकूण 30 स्रोत-एकके आणि 26 वाचक-विभाग' in out and out.count(r'\section{')==26
...
(B/'INPUTS.json').write_text(json.dumps({'input_units':inputs,'scope':'30 source units, 26 reader sections, three complete chapters plus six sections of Size of Sets','conditional_decisions':decisions,'source_issues':'Three Relations, five Functions and five Size of Sets editorial notes; aligned English source is unchanged','notation':'Function macros copied from frozen upstream/open-logic-config.sty; composition applies first argument then second'},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
print(json.dumps({'prepared':'build/core/openlogic-mr-core.tex','units':len(inputs),'sections':26,'sha256':hashlib.sha256(out.encode()).hexdigest()}))
