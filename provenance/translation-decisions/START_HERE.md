...
OpenLogic translation through OLP-0121. It covers 118/722 source units,
11 complete chapters, 254 applied decisions and
7,402 current decision-level occurrence records. The remaining 604 units are untranslated.
...
- `DECISION_OCCURRENCES.csv` has one UTF-8 row for each of 7,402 occurrences.
...
spans and excerpts. 0 prospective terminology records ()
remain in the backward-compatible legacy ledger but are deferred
from `DECISIONS.json` because they have no occurrence in the current 118-unit
