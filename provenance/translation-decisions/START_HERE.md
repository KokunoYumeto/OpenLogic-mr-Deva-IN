...
OpenLogic translation through OLP-0088. It covers 85/722 source units,
8 complete chapters, 228 applied decisions and
5,829 current decision-level occurrence records. The remaining 637 units are untranslated.
...
- `DECISION_OCCURRENCES.csv` has one UTF-8 row for each of 5,829 occurrences.
...
spans and excerpts. 2 prospective terminology records (`T013`, `T185`)
remain in the backward-compatible legacy ledger but are deferred
from `DECISIONS.json` because they have no occurrence in the current 85-unit
