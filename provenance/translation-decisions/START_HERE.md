...
OpenLogic translation through OLP-0068. It covers 65/722 source units, 
8 complete chapters, 209 applied decisions and 
4,690 exact current occurrences. The remaining 657 units are untranslated.
...
- `DECISION_OCCURRENCES.csv` has one UTF-8 row for each of 4,690 occurrences.
...
spans and excerpts. 2 prospective terminology records (`T013`, `T171`) 
remain in the backward-compatible legacy ledger but are deferred 
from `DECISIONS.json` because they have no occurrence in the current 65-unit 
